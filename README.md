...
| `deadlight/` | DEADLIGHT — a WebGPU jump-scare game. Seeded procedural hospital sublevel, a scare director, mannequins that only move off-screen. Every mesh is generated by Tripo, including the rigged and animated creature. |
...
development. (`deadlight/` must be served rather than opened from disk —
WebGPU is only exposed in a secure context, and `http://localhost` is one
while `file://` is not.)

For `ocean/`, the capture and measurement harness lives in `ocean/tools/`:
