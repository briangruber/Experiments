...
import { toTrianglesDrawMode, clone } from './GLTFLoaderDeps.js';
...
class GLTFParser {
	...
	constructor( json = {}, options = {} ) {
		...
		// LOCAL CHANGE: ImageBitmapLoader fetches the blob: URL it is given, and
		// fetch of any URL is a connect-src request. The published single-file
		// page is served under `connect-src 'none'`, so every embedded texture
		// fails there and nowhere else. Setting GLTFLoader.USE_IMAGE_BITMAP to
		// false takes the <img> path instead, which is an img-src request.
		if ( GLTFLoader.USE_IMAGE_BITMAP === false || typeof createImageBitmap === 'undefined' || ( isSafari && safariVersion < 17 ) || ( isFirefox && firefoxVersion < 98 ) ) {
