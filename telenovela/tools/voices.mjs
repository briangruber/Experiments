#!/usr/bin/env node
// Record the cast. Every line in src/dialogue.js is spoken in Spanish by that
// character's voice, then measured, and the measurements are written back to
// src/dialogue-timing.js.
//
...
const { LINES, VOICES } = await import(new URL('../src/dialogue.js', import.meta.url));
...
const measured = await page.evaluate(
  ([ids, hz]) => window.__measure(ids, hz),
  [wanted.map((l) => l.id), ENV_HZ],
);
await browser.close();
server.close();
...
await writeFile(join(ROOT, 'src/dialogue-timing.js'), `// GENERATED by tools/voices.mjs — do not edit.
//
