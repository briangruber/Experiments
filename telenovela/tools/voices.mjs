...
//   node tools/voices.mjs --episode e02-...     # another episode's script
//
// Every recorded clip gets a sidecar, voice/<id>.mp3.hash — the hash of the
// text and voice settings the clip was rendered from (see voice-hash.mjs) —
// which is how tools/pipeline.mjs knows a clip is still current without
// calling the API to find out.
//
...
import { lineHash, voiceSettings } from './voice-hash.mjs';

const ROOT = resolve(dirname(fileURLToPath(import.meta.url)), '..');
const args = process.argv.slice(2);
const opt = (n, d) => { const i = args.indexOf('--' + n); return i >= 0 ? args[i + 1] : d; };
const EP = opt('episode', 'e01-corazon');
const OUT = join(ROOT, `episodes/${EP}/voice`);
...
async function tts(line) {
  ...
  const body = {
    ...
    language_code: 'es',
    // Built by the same function the pipeline hashes, so what is sent and
    // what the sidecar claims was sent are the same object by construction.
    voice_settings: voiceSettings(v),
  };
...
if (!MEASURE_ONLY) {
  ...
  for (const line of LINES) {
    ...
    // The sidecar records what this clip was rendered from, so the pipeline's
    // staleness check stays honest for clips recorded from here on.
    await writeFile(path + '.hash', lineHash(line, VOICES) + '\n');
    made++;
...
const measured = await page.evaluate(
  ...
  [wanted.map((l) => ({ id: l.id, url: `/episodes/${EP}/voice/${l.id}.mp3` })), ENV_HZ],
);
await browser.close();
server.close();
...
await writeFile(join(ROOT, `episodes/${EP}/dialogue-timing.js`), `// GENERATED by tools/voices.mjs — do not edit.
//
