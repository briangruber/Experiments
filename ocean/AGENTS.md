...
1. **Every subsystem has a check; run the one you touched.** All are headless
   ...
   | the sea dragon's real particle spray where it breaches, its breach burst, or its wake trail (`dragonSpray`/`dragonImpact`/`dragonWakeT` in `three-main.js`, `this.climb` in `seadragon.js`, the `ctx.craftBeam/craftLen` overrides in `spray.js`) | `node tools/run-probe.mjs prototypes/dragon-spray-probe.html` — tests at the animal's REAL minimum staging depth (the gates key off the back, `pos[1] + dragonTopY`, never the origin — the origin can't come near the surface, and a gate reading it never fires outside a rigged probe), that the spawn footprint spans the animal rather than the ski (particles inside the mesh depth-test away invisibly), that the burst spikes then fades, that the wake goes live only while it works the surface, and that a vehicle riding at the same time takes both emitters back |
