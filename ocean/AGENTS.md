...
1. **Every subsystem has a check; run the one you touched.** All are headless
   ...
   | the sea dragon's real particle spray where it breaches, its breach burst, or its wake trail (`dragonSpray`/`dragonImpact`/`dragonWakeT`/`wakeHold`/`dragonHighPoint()` in `three-main.js`, `this.climb` in `seadragon.js`, the `ctx.craftBeam/craftLen` overrides in `spray.js`, the `dims` override in `wake-driver.js`) | `node tools/run-probe.mjs prototypes/dragon-spray-probe.html` — tests at the animal's REAL minimum staging depth (the gates key off the PITCHED high point, `dragonHighPoint()`, never the origin — the origin can't come near the surface, and a porpoising body's nose/tail ride up to `sdLength/2·sin(pitch)` above the level posture), that its real swim speed reaches the same `craftPlaneSpeed..Full` gate the vehicles obey, that the spawn footprint and the wake track's render width (`sdWakeBeam`, via `wake.uniforms` dims) span the animal rather than the ski, that the burst spikes then fades, that diving lets the old wake FADE over `wakeLife` instead of vanishing (the `wakeHold` latch, captured off the real `wake.uniforms` call), and that a vehicle riding at the same time takes both emitters back |
