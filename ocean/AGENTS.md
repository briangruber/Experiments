...
1. **Every subsystem has a check; run the one you touched.** All are headless
   ...
   | the sea dragon's real particle spray where it breaches (`ctx.craft*`'s `dragonSpray` branch) | `node tools/run-probe.mjs prototypes/dragon-spray-probe.html` — checks the shared emitter's own uniforms, and that a vehicle riding at the same time takes priority over the animal |
