...
function buildHarbor(){
  ...
  b.island(235,-735,92,72,28,4.1);
  // Layered village climbing the left hillside.
  const homes=[
    [-505,8,-595,1.35,0],[-455,10,-575,1.0,1],[-410,14,-585,.9,0],[-550,15,-655,.95,1],
    [-485,24,-665,.85,0],[-425,30,-650,1.1,1],[-370,37,-670,.85,0],[-520,35,-735,.78,0],
    [-455,45,-745,.82,1],[-390,52,-735,.72,0],[-320,42,-700,.85,0],[-290,25,-650,.9,1],
  ];
  homes.forEach(([x,y,z,s,blue])=>b.house(x,y,z,s,blue));
  // Harbor piers.
  for(let j=0;j<8;j++) b.box(-425+j*13,2,-545,11,1.1,48,C.wood);
  for(let j=0;j<10;j++) b.cylinder(-425+j*13,-2,-526,1.0,7,C.trunk,7);
  // Pines create the dense concept-art silhouette.
  const trees=[
    [-595,24,-680,1.5],[-565,36,-720,1.3],[-535,47,-780,1.5],[-490,57,-805,1.1],
    [-435,62,-825,1.4],[-375,67,-805,1.3],[-320,59,-790,1.5],[-275,44,-750,1.2],
    [-235,31,-700,1.3],[-570,30,-610,1.0],[-335,48,-620,1.1],[-270,27,-590,1.0],
    [-470,63,-705,1.0],[-405,70,-710,1.1],
  ];
  trees.forEach(t=>b.pine(...t));
  // Lighthouse island to the right, with a warm rotating-room impression.
  b.cylinder(235,17,-735,9,45,C.white,12);
  b.cylinder(235,17,-735,11,7,C.red,12);
  b.cylinder(235,42,-735,9,8,C.red,12);
  b.cylinder(235,62,-735,12,4,C.roof,12);
  b.cylinder(235,55,-735,8.5,7,C.window,12);
  b.cone(235,66,-735,13,9,C.roof,12);
  b.box(235,20,-726,4,7,.5,C.cliff);
  for(const [x,z,s] of [[185,-735,1.2],[285,-750,1.1],[260,-700,.85],[205,-775,.9]]) b.pine(x,24,z,s);
  return b;
}
