#!/usr/bin/env node
// Bake a GLB into src/barrel-asset.js — a small ES module the app imports and
// the bundler inlines, so the artifact stays self-contained with no fetches.
//
//   node tools/barrel.mjs <model.glb>
//
...
const TEX = 256;          // baked texture size, px
const OUT = new URL('../src/barrel-asset.js', import.meta.url);
if (!SRC) { console.error('usage: barrel.mjs <model.glb>'); process.exit(1); }
...
const out = `// Generated by tools/barrel.mjs — do not edit.
...
console.log(`wrote src/barrel-asset.js  ${(out.length / 1024) | 0} KiB`);
