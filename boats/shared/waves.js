// Gerstner wave field. One table, three consumers: the ocean vertex shader,
// the CPU buoyancy sampler, and the server's spawn placement. The GLSL is
// generated from the same numbers so the water you see is the water you float
// on -- a hull that bobs half a metre out of sync with the crest reads as a bug
// from the first second of play.

export const GRAVITY = 9.81;

// dir is normalised on load. len is the wavelength in metres, amp the crest
// height, steep the Gerstner choppiness (0 = sine, 1 = cusped), speed a
// multiplier on the physically correct phase velocity.
export const WAVES = [
  { dir: [1.00, 0.18], len: 96, amp: 0.72, steep: 0.62, speed: 1.00 },
  { dir: [0.72, -0.69], len: 57, amp: 0.46, steep: 0.58, speed: 1.04 },
  { dir: [-0.35, 0.94], len: 33, amp: 0.28, steep: 0.55, speed: 0.96 },
  { dir: [0.94, 0.34], len: 19, amp: 0.16, steep: 0.50, speed: 1.08 },
  { dir: [-0.82, -0.57], len: 11.5, amp: 0.085, steep: 0.45, speed: 1.02 },
  { dir: [0.31, 0.95], len: 6.5, amp: 0.042, steep: 0.40, speed: 1.06 },
];

// Precomputed per-wave constants: [dx, dz, k, amp, qa (steep*amp), omega].
const W = WAVES.map((w) => {
...
export function waveDisplace(px, pz, t, out = {}) {
  ...
  tx += 1;
  bz += 1;
  // n = binormal x tangent, with the axes arranged so +y points at the sky.
  let nx = by * tz - bz * ty;
  let ny = bz * tx - bx * tz;
  let nz = bx * ty - by * tx;
...
/**
 * Height of the water directly above/below a world XZ. Gerstner waves move
 * points horizontally too, so we invert that displacement with a few fixed
 * point steps -- three is plenty at these steepnesses.
 */
...
export function waterHeight(wx, wz, t) {
  return sampleWater(wx, wz, t, _h).y;
}

/** GLSL translation of waveDisplace(), with the wave table baked in. */
export function waveGLSL() {
  return `
const int WAVE_COUNT = ${W.length};
// Packed as (dirX, dirZ, k, amp) and (steepAmp, omega).
