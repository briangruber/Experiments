#!/usr/bin/env node
// What does the SURFACE SIM hold at a fixed point as the boat runs away?
// Reads the sim's own texture, not the field's -- foam / air / fresh.
//
...
const qs = new URLSearchParams({ prewarm: '30', 'boat.speed': SPEED, 'boat.turnRate': '0' });
...
const rows = await page.evaluate(({ lat, secs }) => {
  ...
  const { wake, surface, renderer, state, stepSim, get, spray, feedSurface } = window.__wake;
  ...
  const bake = () => {
    ...
    wake.update(state.t);
    // The frame loop feeds and steps the surface after the field; so do we.
    spray.step(1 / 30, () => 0);
    feedSurface(1 / 30);
    surface.step(1 / 30);
  };
  ...
  const out = [];
  const read = () => {
    const rt = surface.rt[surface.i], N = rt.width;
    const buf = new Uint16Array(N * N * 4);
    renderer.readRenderTargetPixels(rt, 0, 0, N, N, buf);
    const u = (wx - surface.center.x) / surface.extent + 0.5;
    const v = -(wz - surface.center.y) / surface.extent + 0.5;
    ...
    return { foam: h2f(buf[o]), churn: h2f(buf[o + 1]), bub: h2f(buf[o + 2]) };
  };
  const dt = 1 / 30;
...
console.log(`SURFACE SIM at ONE fixed world point ${LAT} m off the track, as she runs away.\n`);
console.log('  t(s)  astern(m)     foam      air   (sim texture)');
let peak = 0;
