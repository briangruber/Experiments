...
const out = await page.evaluate(async () => {
  ...
  const withPropagate = fieldMax();
  w.set('kelvin.propagate', 0);
  w.wake.update(w.state.t);   // the SIM clock in seconds, not wall ms
  const withoutPropagate = fieldMax();
  w.set('kelvin.propagate', 1);
  let ahead = null;
  try {
  ...
  return { cruising, stopped, withPropagate, withoutPropagate, ahead };
});
