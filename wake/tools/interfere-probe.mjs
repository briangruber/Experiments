...
await page.goto(`http://127.0.0.1:${server.address().port}/?prewarm=25&boat.speed=8&kelvin.interfere=1.6&kelvin.amp=0.5&shore.on=0`, { waitUntil: 'load' });
...
const out = await page.evaluate(async () => {
  ...
  const stopped = snap('steady 8 m/s');
  // Bake cost, both ways. Several bakes each so one slow frame cannot decide it.
  const timeBake = (v) => {
    w.set('kelvin.interfere', v);
    w.wake.update(w.state.t);            // warm
    const t0 = performance.now();
    for (let i = 0; i < 8; i++) w.wake.update(w.state.t);
    return +((performance.now() - t0) / 8).toFixed(2);
  };
  const bakeOff = timeBake(0);
  const bakeOn = timeBake(1.6);
