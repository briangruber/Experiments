...
const RIBBON_FRAG = /* glsl */`
  ...
  uniform vec4  uOutMask;
  uniform float uDepFresh;
  ...
  void main(){
    ...
    vec4 outC = vec4((foam + cavOut * uCavFoam) * edge,
                     height * edge,
                     (bubOut * surfaced + cavOut) * edge,
                     (bubOut + cavOut * 1.35) * edge);
    // THE DEPOSIT PASS uses this same shader with a mask, so there is exactly
    // one description of what the wake looks like and no second copy to drift.
    // uDepFresh limits it to water the hull has only just passed: foam is MADE
    // as she goes by, and everything after that is the deposit's own business.
    if (uDepFresh > 0.0) outC *= exp(-age / uDepFresh);
    gl_FragColor = outC * uOutMask;
  }
`;
...
export class WakeField {
  constructor(renderer, size = 1024) {
    ...
    // ---------------------------------------------------------------------
    // THE DEPOSIT: foam that lives on the water.
    //
    // Everything above is a RECONSTRUCTION. The field is re-derived from the
    // path every frame, so the wake is a function of where the boat has been
    // and how fast it is going NOW -- which means anything that changes the
    // path or its parameterisation reshapes the whole existing wake
    // retroactively. Reverse, and the anchor moves, and every arc moves with
    // it. That is not a bug to be found and fixed; it is what "reconstructed"
    // means, and it will keep producing bugs of that shape.
    //
    // This is the other half: a world-locked buffer that is only ever ADDED to
    // and decayed. Nothing re-derives it, so nothing the boat does later can
    // reach back and move what is already lying on the water.
    //
    // The one real objection on record -- that advection was tried and lost to
    // numerical diffusion -- does not apply, because there is no advection
    // here. Foam is laid where it is made and then simply fades. And the
    // resampling that would normally blur a scrolling buffer does not happen
    // either: focus() already snaps the centre to whole multiples of four
    // texels, so following the boat is an INTEGER shift and the copy is exact.
    const dep = () => new THREE.WebGLRenderTarget(size, size, {
    ...
    this.dep = [dep(), dep()];
    this.depI = 0;
    this.depCenter = new THREE.Vector2(0, 0);
    this.depExtent = this.extent;
    this.depReset = true;

    this.depUniforms = {
      uPrev: { value: this.dep[1].texture },
      uShift: { value: new THREE.Vector2(0, 0) },
      uDecay: { value: 1 },
    };
    this.depScene = new THREE.Scene();
    this.depCam = new THREE.OrthographicCamera(-1, 1, 1, -1, 0, 1);
    this.depScene.add(new THREE.Mesh(new THREE.PlaneGeometry(2, 2),
      new THREE.ShaderMaterial({
        uniforms: this.depUniforms,
        vertexShader: /* glsl */`
          varying vec2 vUv;
        ...
        fragmentShader: /* glsl */`
          ...
          varying vec2 vUv;
          void main(){
            // Follow the boat by resampling the previous frame at an offset.
            // The offset is a whole number of texels (see focus()), so this is
            // a copy rather than a filter and the foam does not smear as the
            // window walks over it.
            vec2 uv = vUv + uShift;
            gl_FragColor = any(lessThan(uv, vec2(0.0))) || any(greaterThan(uv, vec2(1.0)))
              ? vec4(0.0) : texture2D(uPrev, uv) * uDecay;
          }
        `,
  ...
  _buildGeometry() {
    ...
    this.uniforms = {
      ...
      uOutMask: { value: new THREE.Vector4(1, 1, 1, 1) }, uDepFresh: { value: 0 },
  ...
  /** The texture the water samples for deposited foam. */
  depositTexture() { return this.dep[this.depI].texture; }

  /**
   * Lay down this frame's foam and let what is already there fade.
   *
   * Three steps, in order: follow the boat (an exact integer-texel copy), fade
   * what was already down, then add what the hull is making right now. The
   * ribbon shader draws that last step -- the same shader the field uses, with
   * a mask -- so there is one description of what a wake looks like rather than
   * two that can drift apart.
   */
  deposit(dt) {

    const amount = get('field.deposit');
    if (amount <= 0.0001) return;
    const r = this.renderer;
    const prevTarget = r.getRenderTarget();
    const prev = this.dep[this.depI];
    const next = this.dep[this.depI ^ 1];

    // A CHANGE OF EXTENT CANNOT BE FOLLOWED, only rescaled, and rescaling a
    // buffer whose whole point is that it is never resampled would blur every
    // patch of foam in it. So the window size is fixed while foam is on it and
    // a change simply starts again.
    if (Math.abs(this.depExtent - this.extent) > 1e-3) {
      this.depExtent = this.extent;
      this.depReset = true;
    }
    if (this.depReset) {
      r.setClearColor(0x000000, 0);
      for (const t of this.dep) { r.setRenderTarget(t); r.clear(true, false, false); }
      this.depCenter.copy(this.center);
      this.depReset = false;
    }

    // Step 1 and 2: scroll and fade, in one pass. The shift is in UV, and v is
    // flipped because the bake looks down with up = (0, 0, -1), so +Z in the
    // world runs DOWN the texture.
    const e = Math.max(this.depExtent, 1e-3);
    this.depUniforms.uPrev.value = prev.texture;
    this.depUniforms.uShift.value.set(
      (this.center.x - this.depCenter.x) / e,
      -(this.center.y - this.depCenter.y) / e);
    // Half-life in seconds, as a rate: a patch left alone loses half of itself
    // every `life` seconds, which is a decay you can reason about rather than a
    // per-frame multiplier that changes meaning with the frame rate.
    const life = Math.max(get('field.depositLife'), 0.5);
    this.depUniforms.uDecay.value = Math.pow(0.5, dt / life);
    r.setRenderTarget(next);
    r.setClearColor(0x000000, 0);
    r.clear(true, false, false);
    r.render(this.depScene, this.depCam);

    // Step 3: add what she is making now. Foam and bubbles only -- height is a
    // WAVE, it travels and it is not something the water keeps, so it is masked
    // out here and left entirely to the reconstruction.
    const g = amount * dt;
    const u = this.uniforms;
